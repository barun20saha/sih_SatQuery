...
export default function ReportsPage() {
  ...
  const handleDownloadReport = (title) => {
    // Generate dummy PDF content (just basic text string as blob)
    const dummyData = `Intelligence Report: ${title}\n\nGenerated by SatQuery AI.\nThis is a dummy PDF file containing sample data.`;
    const blob = new Blob([dummyData], { type: 'application/pdf' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${title ? title.replace(/\s+/g, '_') : 'Intelligence'}_Report.pdf`;
